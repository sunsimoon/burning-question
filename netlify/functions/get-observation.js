...
exports.handler = async (event, context) => {
    ...
    try {
        const { step, stepTitle, currentResponse, previousResponses, previousObservations } = JSON.parse(event.body);
                ...
                res.on('end', () => {
                    if (res.statusCode === 200) {
                        ...
                        resolve({
                            statusCode: 200,
                            headers,
                            body: JSON.stringify({ observation: response.content[0].text })
                        });
                    } else {
                        resolve({
                            statusCode: 500,
